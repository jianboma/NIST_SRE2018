function [v, E]=estimate_y_and_v_diag_cov(F, N, S, m, E, d, v, u, z, y, x, spk_ids)
...
SumSs = zeros(1,size(S,2));
SumNs = zeros(1,dim*size(N,2));
if nargout > 1
    for c=1:size(N,2)
        A{c} = zeros(size(v,1));
    end
...
v=update_v(A, C);
E = (SumSs-sum(C.*v,1))./SumNs; %update the covariance
end
