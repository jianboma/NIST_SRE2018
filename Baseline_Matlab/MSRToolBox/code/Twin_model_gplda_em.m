...
% revised to twin model GPLDA by Jianbo Ma
...
% for spk = 1 : nspks
%     % Speaker indices
%     session_short = spk_counts_short(spk);
%     idx_short = cnt_short : ( cnt_short - 1 ) + spk_counts_short(spk);
%     cnt_short  = cnt_short + spk_counts_short(spk);
%     Data_short = data_short(:, idx_short);
%     
%     session_long = spk_counts_long(spk);
...
% %     times = floor(session_short/session_long);
% %     mod_left = mod(session_short,session_long);
% %     data_long_duplication = repmat(Data_long,1,times);
% %     num_mod_randm = randi([1 session_long],mod_left,1);
% %     data_long_duplication = [data_long_duplication Data_long(:,num_mod_randm)];
% %     data_long_duplication = data_long_duplication(:,num_re_order_randm);
%     data_long_duplication = Data_long(:,num_re_order_randm);
%     data_augment = [Data_short;data_long_duplication];
%     data = [data data_augment];
%     data_long_duplication = [];
%     data_augment = [];
% end
spk_labs_short_uniq = unique(spk_labs_short);
spk_labs_long_uniq = unique(spk_labs_long);
for spk = 1 : nspks
    % Speaker indices
    ...
    whlong = ismember(spk_labs_long,spk_labs_long_uniq{spk});
...
%     [Phi, Sigma] = maximization_plda_test(data, Ey, Eyy);
...
function [Phi, Sigma] = maximization_plda_test(data, Ey, Eyy)
% ML re-estimation of the Eignevoice subspace and the covariance of the
% residual noise (full).
% nsamples = size(data, 2);
% Data_sqr = data(1:size(data,1)/2,:) * data';
% Phi      = data * Ey' / (Eyy);
% Sigma    = 1/nsamples * (Data_sqr - (Phi * Ey) * data');
data_short = data(1:size(data,1)/2,:);
data_long = data(size(data,1)/2+1:size(data,1),:);
nsamples_short = size(data_short, 2);
Data_sqr_short = data_short * data_short';
Phi_short      = data_short * Ey' / (Eyy);
Sigma_short    = 1/nsamples_short * (Data_sqr_short - (Phi_short * Ey) * data_short');

nsamples_long = size(data_long, 2);
Data_sqr_long = data_long * data_long';
Phi_long      = data_long * Ey' / (Eyy);
Sigma_long    = 1/nsamples_long * (Data_sqr_long - (Phi_long * Ey) * data_long');
